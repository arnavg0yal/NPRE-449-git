...
plotted = ["Saturation Temperature", "Wall Temperature", "Fluid Temperature"]
...
if option == 1 :
    ...
    for numOfPoints in versions:
        ...
        for i in range(1, len(zs)):
            print(f"{i}, {Ps[-1]/(10**6):0.2e}MPa, {Tsats[-1]:0.2f}C, {T_ws[-1]:0.2f}C, {T_f_s[-1]:0.2f}C, {Xes[-1]:0.2e}, {mus[-1]}")
            ...
            if Xes[-1] < 0:
                ...
                deltaPcurrent = (2*f1phase*(G**2)/(rhos[-1]*D_equiv)) + (rhos[-1]*g)
            else:
                Res.append(reynolds(presQualProp(Ps[-1], Xes[-1]*100, "dynVisc")))
                f2phase, mux = frictionfactor2phase(Res[-1], Ps[-1], Xes[-1])
                mus.append(mux)
                frics.append(f2phase)
                vfg = calcvfg(pres=Ps[-1])
                vf  = presQualProp(Ps[-1], 0, "vol")
                deltaPcurrent = (G**2)*vfg*((Xes[-1]-Xes[-2])/deltaz) + (f2phase*(G**2)*(vfg*Xes[-1] + vf)*2/D_equiv) + (g/(vfg*Xes[-1]+vf))
...
if "Wall Temperature" in plotted:
    ax.plot(T_ws, zs, label="Outer Clad Surface Temperature")
